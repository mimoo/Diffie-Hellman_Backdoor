...
\todo{explain the attack, say that it's usually hard to use}
...
A second idea would have been to smooth enable the requirement for the Pohlig-Hellman algorithm to work. This can be done by fixing a prime modulus $p$ such that $p-1$ is B-smooth \todo{definition of B-smooth, should include it in the previous section tho} with B small enough for discrete logarithms in bases of order B to be possible.\\
...
Ah \todo{Should add a proof here that our solution works _all the time_, I think it's because we have many different solutions for the many values modulo $q_2$ and $p_2$, but the map is not injective} By reducing the discrete logarithm problem $y = g^x$ modulo $p$ and $q$ with our new backdoored generator, we can compute $x$ modulo $p-1$ and $q-1$ more easily and then recompute an equivalent secret key modulo $\frac{p-1}{2}(q-1)$. Another solution can be found by adding $\frac{p-1}{2}(q-1)$ to the integer representation of the previous solution. Since $p-1$ and $q-1$ are not coprime we cannot guess the original secret key, but this is usually enough as both found solutions will compute a valid Diffie-Hellman shared key by exponentiating the other peer's public key.\\
